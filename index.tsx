...
let v0_3_0_dom: any = {}, v1_0_0_dom: any = {}, shared_dom: any = {}, auth_dom: any = {};
...
function v0_3_0_handleJobClick(e) {
    ...
    if (id === 'starcatching') {
        if (gameState.shared.hasTwilightTrophy) { v0_3_0_showNotification('이미 황혼의 트로피를 획득했습니다!', true); return; }
        const success = Math.random() < 0.25; 
        if (success) { gameState.shared.hasTwilightTrophy = true; v0_3_0_showNotification('황혼의 트로피를 획득했습니다!', false); v0_3_0_updateTrophyDisplay(); if (v0_3_0_dom.jobStarcatching) v0_3_0_dom.jobStarcatching.classList.add('hidden'); } 
...
function v0_3_0_handleBuy3DCube() { const state = gameState.v0_3_0; if (state.userCash >= 1000000) { state.userCash -= 1000000; state.isCubePurchased = true; v0_3_0_restoreUIState(); v0_3_0_showNotification('3D 큐브 구매 완료!', false); v0_3_0_updateUI(); saveGameState().catch(e => console.error("Save failed:", e)); } else { v0_3_0_showNotification('현금이 부족합니다.', true); } }
function v0_3_0_handleUpgradePrism() { const state = gameState.v0_3_0; if (state.userPrisms >= 100) { state.userPrisms -= 100; state.isPrismUpgraded = true; v0_3_0_restoreUIState(); v0_3_0_showNotification('프리즘 업그레이드 완료!', false); if (cubeMaterial) cubeMaterial.color.set(0xf472b6); v0_3_0_updateUI(); saveGameState().catch(e => console.error("Save failed:", e)); } else { v0_3_0_showNotification('프리즘이 부족합니다.', true); } }
function v0_3_0_handleUpgradeLunarBlessing() { const state = gameState.v0_3_0; if (state.userLunar >= 200) { state.userLunar -= 200; state.isLunarBlessed = true; v0_3_0_restoreUIState(); v0_3_0_showNotification('달빛 강화 완료!', false); v0_3_0_updateUI(); saveGameState().catch(e => console.error("Save failed:", e)); } else { v0_3_0_showNotification('루나가 부족합니다.', true); } }
function v0_3_0_handleSleep() { const state = gameState.v0_3_0; if (state.isSleeping) return; state.isSleeping = true; v0_3_0_showNotification('수면을 시작합니다...', false); if (v0_3_0_dom.sleepButton) v0_3_0_dom.sleepButton.textContent = '수면 중...'; const hoursToSleep = (32 - v0_3_0_gameTime.getHours()) % 24; const sleepTime = hoursToSleep * 60 * 250; setTimeout(() => { state.isSleeping = false; v0_3_0_gameTime.setHours(8, 0, 0, 0); v0_3_0_showNotification('좋은 아침입니다!', false); if (v0_3_0_dom.sleepButton) v0_3_0_dom.sleepButton.textContent = '수면'; v0_3_0_updateUI(); saveGameState().catch(e => console.error("Save failed:", e)); }, sleepTime); }
...
function v1_0_0_migrateData() {
    ...
    v1_0_0_showNotification('v.0.4.0 데이터 마이그레이션 완료! $PRISM이 $ENERGY로 전환되었습니다.', false);
}
...
function v1_0_0_updateComputerUI() {
    ...
    const tiers = [
        { name: '컴퓨터 없음', income: 0, cost: 500000, next: 'Tier 1 구매' },
        { name: 'Tier 1: 구형 컴퓨터', income: 100, cost: 2000000, next: 'Tier 2 업그레이드' },
        { name: 'Tier 2: 최신형 컴퓨터', income: 500, cost: 10000000, next: 'Tier 3 업그레이드' },
        { name: 'Tier 3: 전문가용 워크스테이션', income: 2500, cost: 50000000, next: 'Tier 4 업그레이드' },
        { name: 'Tier 4: 양자 컴퓨터', income: 10000, cost: Infinity, next: '최고 티어' }
    ];
    v1_0_0_dom.computerTierText.textContent = tiers[tier].name;
    v1_0_0_dom.computerStatsText.textContent = `초당 +${tiers[tier].income.toLocaleString()} KRW 생산`;
    if (tier < 4) {
    ...
    for (const weather in WEATHER_DATA) {
        ...
        el.innerHTML = `
            ...
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1">${hasExperienced ? WEATHER_DATA[weather].description : '경험하지 않은 날씨입니다.'}</p>
            </div>
        `;
...
function v1_0_0_populateTradeUI() {
    ...
    coins.forEach(coin => {
        ...
        el.innerHTML = `
            ...
            <input type="number" id="v1.0.0-amount-input-${coin.id}" value="1" min="1" class="w-full bg-gray-800 text-white p-2 rounded mt-2 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-${coin.color}-500" placeholder="${coin.name} 수량">
            <div class="grid grid-cols-2 gap-4 mt-4">
                <button id="v1.0.0-buy-button-${coin.id}" class="w-full bg-green-600 hover:bg-green-700 font-bold p-3 rounded-lg">매수</button>
                <button id="v1.0.0-sell-button-${coin.id}" class="w-full bg-red-600 hover:bg-red-700 font-bold p-3 rounded-lg">매도</button>
            </div>
        `;
...
function v1_0_0_populateShopItems() {
    ...
    const items = [
        { id: 'digitalClock', name: '디지털 시계', desc: '게임 내 시간과 날씨를 화면에 표시합니다.', cost: 1000000 },
        { id: 'weatherAlmanac', name: '날씨 도감', desc: '지금까지 경험한 날씨와 효과를 기록합니다.', cost: 5000000 },
        { id: 'bed', name: '침대', desc: '밤에 잠을 자서 시간을 빠르게 보낼 수 있습니다.', cost: 2500000 },
    ];
    items.forEach(item => {
        const el = document.createElement('div');
        el.className = 'bg-gray-600 p-4 rounded-lg';
        el.innerHTML = `
            ...
            <button id="v1.0.0-buy-${item.id}" class="w-full bg-blue-600 hover:bg-blue-700 font-bold py-2 px-4 rounded-lg">
                ${gameState.v1_0_0.shopItems[item.id] ? '보유중' : `${item.cost.toLocaleString()} KRW`}
            </button>
        `;
        container.appendChild(el);
// FIX: Cast button to HTMLButtonElement to access 'disabled' property and add null check for safety.
        const button = document.getElementById(`v1.0.0-buy-${item.id}`) as HTMLButtonElement;
        if (button) {
...
function v1_0_0_handleShopBuy(itemId, cost) {
    ...
    if (state.userCash >= cost) {
        ...
        v1_0_0_showNotification(`${itemId} 구매 완료!`, false);
        v1_0_0_populateShopItems(); v1_0_0_restoreUIState(); saveGameState();
    } else { v1_0_0_showNotification('현금이 부족합니다.', true); }
}
function v1_0_0_getNewPrice(currentPrice, weather) {
    let riseProb = 0.51, periodAdj = 1.0;
    if (weather === '맑음') riseProb += 0.005; else if (weather === '비') periodAdj += 0.05; else if (weather === '구름') periodAdj -= 0.05; else if (weather === '산성비') riseProb -= 0.025; else if (weather === '천둥') riseProb -= 0.005; else if (weather === '무지개') riseProb += 0.05;
...
function v1_0_0_priceUpdateLoop() {
    ...
    let interval = 2000;
    if (state.weather === '비') interval *= 1.05; if (state.weather === '구름') interval *= 0.95;
...
function v1_0_0_gameLoop() {
    ...
    if (state.weatherCounter >= 120) { // 30초마다 날씨 변경 (250ms * 120)
        ...
        let possibleWeathers = ['맑음', '비', '구름'];
        if (Math.random() < 0.1) possibleWeathers.push('산성비');
        if (Math.random() < 0.1) possibleWeathers.push('천둥');
        if (state.nextWeatherIsRainbow || Math.random() < 0.05) { state.weather = '무지개'; state.nextWeatherIsRainbow = false; } 
        else if (state.lastWeather === '비' && Math.random() < 0.1) { state.nextWeatherIsRainbow = true; state.weather = possibleWeathers[Math.floor(Math.random() * possibleWeathers.length)]; } 
    ...
    if (state.weather === '천둥' && Math.random() < 0.005) state.isInternetOutage = true; 
    else if (state.isInternetOutage && state.weather !== '천둥') state.isInternetOutage = false;
...
function v1_0_0_handleTrade(type, coinId) {
    const state = gameState.v1_0_0;
    const amountInput = document.getElementById(`v1.0.0-amount-input-${coinId}`) as HTMLInputElement;
    if (!amountInput) return; const amount = parseInt(amountInput.value); if (!(amount > 0)) return;
    ...
    if (type === 'buy') {
        if (state.userCash >= cost) { state.userCash -= cost; state[coinData[coinId].balance] += amount; v1_0_0_showNotification(`${amount} ${coinUpper} 매수!`, false); } 
        else { v1_0_0_showNotification('현금 부족', true); return; }
    } else {
        if (state[coinData[coinId].balance] >= amount) { state.userCash += cost; state[coinData[coinId].balance] -= amount; v1_0_0_showNotification(`${amount} ${coinUpper} 매도!`, false); } 
        else { v1_0_0_showNotification(`${coinUpper} 부족`, true); return; }
    }
    v1_0_0_updateUI(); saveGameState();
}
function v1_0_0_handleBuy3DCube() { const state = gameState.v1_0_0; if (state.userCash >= 1000000) { state.userCash -= 1000000; state.isCubePurchased = true; v1_0_0_restoreUIState(); v1_0_0_showNotification('3D 큐브 구매 완료!', false); v1_0_0_updateUI(); saveGameState(); } else { v1_0_0_showNotification('현금이 부족합니다.', true); } }
function v1_0_0_handleComputerUpgrade() {
    ...
    if (state.userCash >= cost) {
        ...
        v1_0_0_showNotification(`컴퓨터 업그레이드 완료! (Tier ${state.computerTier})`, false);
...
function v1_0_0_handleSleep() {
    ...
    v1_0_0_showNotification('수면을 시작합니다... (8배속)', false);
    if (v1_0_0_dom.sleepButton) v1_0_0_dom.sleepButton.textContent = '수면 중...';
    ...
    const sleepInterval = setInterval(() => {
        ...
        if (sleepCounter >= 8 * 120) { // Simulate 8 hours (8 * 30 seconds)
            ...
            v1_0_0_showNotification('좋은 아침입니다!', false);
            if (v1_0_0_dom.sleepButton) v1_0_0_dom.sleepButton.textContent = '수면';
            v1_0_0_updateUI();
            saveGameState();
        }
...
function handleCodeSubmit() {
    const input = document.getElementById(`${currentVersion}-code-input`) as HTMLInputElement;
    if (!input) return;
    ...
    if (code === 'TWILIGHT' && currentVersion === 'v0.3.0') {
        ...
        rewardGiven = true;
         ...
         showNoti('치트 코드: 모든 코인 업그레이드 잠금 해제!', false);
         rewardGiven = true;
        ...
        showNoti('v.0.4.0 5분 프리뷰가 시작됩니다!', false);
        startPreviewTimer(gameState.shared.v0_4_0_preview_end_time);
        rewardGiven = true;
    } else {
        showNoti('유효하지 않은 코드입니다.', true);
    }
...
function startPreviewTimer(endTime) {
    ...
    previewTimerInterval = window.setInterval(() => {
        ...
        if (remaining <= 0) {
            ...
            switchVersion('v0.3.0'); // 시간이 다 되면 이전 버전으로 강제 전환
            v0_3_0_showNotification('프리뷰 시간이 종료되었습니다.', false);
            saveGameState();
            return;
        }
...
function initCountdownBanner() {
    ...
    const updateCountdown = () => {
        ...
        if (distance < 5 * 60 * 60 * 1000) { // 5시간 전
             banner.classList.remove('hidden');
        } else {
            banner.classList.add('hidden');
            return;
        ...
        if (distance <= 0) {
            ...
            banner.innerHTML = 'v.0.4.0 업데이트가 완료되었습니다! 페이지를 새로고침 해주세요.';
            return;
        }
        ...
        timer.textContent = `${hours}시간 ${minutes}분 ${seconds}초`;
        
        // 1시간 전부터 코드 안내
        if (distance < 1 * 60 * 60 * 1000) {
            codeAnnouncement.classList.remove('hidden');
            codeAnnouncement.textContent = '힌트: 업데이트 후 코드를 입력하여 특별한 보상을 받으세요! 코드: UPGRADE';
        }
...
function init3D() {
    ...
    const animate = () => {
        if (!document.getElementById(`${currentVersion}-canvas-container`)) return; // 컨테이너가 없으면 애니메이션 중지
...
async function loadGameState(uid) {
    try {
        ...
        if (docSnap.exists) {
            ...
            console.log("기존 데이터 없음. 새 게임을 시작합니다.");
            gameState = getInitialGameState();
            // 새 사용자를 위해 초기 상태를 저장
            await saveGameState();
            return false;
...
function getKoreanAuthErrorMessage(errorCode) {
    switch (errorCode) {
        ...
        case 'auth/too-many-requests':
            return '너무 많은 로그인을 시도했습니다. 잠시 후 다시 시도해주세요.';
        default:
...
function handleLogin() {
    const email = (auth_dom.loginEmailInput as HTMLInputElement).value;
    const password = (auth_dom.loginPasswordInput as HTMLInputElement).value;
    auth_dom.errorMessage.textContent = '';

    if (!email || !password) {
        auth_dom.errorMessage.textContent = '이메일과 비밀번호를 입력하세요.';
        return;
    }
...
function handleSignup() {
    const email = (auth_dom.signupEmailInput as HTMLInputElement).value;
    const password = (auth_dom.signupPasswordInput as HTMLInputElement).value;
    auth_dom.errorMessage.textContent = '';

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) {
        auth_dom.errorMessage.textContent = '유효한 이메일 주소를 입력하세요.';
        return;
    }

    if (password.length < 6) {
        auth_dom.errorMessage.textContent = '비밀번호는 6자 이상이어야 합니다.';
        return;
    }
    ...
    auth.createUserWithEmailAndPassword(email, password)
        ...
        .then(() => {
            ...
            v0_3_0_showNotification('회원가입 성공! 로그인 해주세요.', false);
            auth_dom.signupForm.classList.add('hidden');
            auth_dom.loginForm.classList.remove('hidden');
            (auth_dom.loginEmailInput as HTMLInputElement).value = email;
            (auth_dom.loginPasswordInput as HTMLInputElement).value = '';
        })
        .catch(error => {
            auth_dom.errorMessage.textContent = getKoreanAuthErrorMessage(error.code);
        });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            if (window.autosaveInterval) clearInterval(window.autosaveInterval);
            window.autosaveInterval = setInterval(saveGameState, 30000); // 30초마다 자동 저장
